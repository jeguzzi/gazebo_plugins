...
This plugin publish three topics:
...
- `~/contact_state` a [ContactState.msg](https://github.com/jeguzzi/gazebo_plugins/blob/master/msg/ContactState.msg) which inform if the wheels (and the chassis) are touching the terrain;
- `~/power` a
[Power.msg](https://github.com/jeguzzi/gazebo_plugins/blob/master/msg/Power.msg) that contains power and (motor) consumed energy.

Note that the pioneer3at simulation has no motors but a controller that instantaneously change the joint's angular velocity. We estimate power by computing for each wheel w in {fl = front left, fr, bl, br}
